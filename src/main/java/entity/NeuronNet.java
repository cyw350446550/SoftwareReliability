...
public class NeuronNet {
    ...
    /**
     * 下一层网络残差, data[i][j] = 第i层, 第j神经元的残差, i = 1 ... layer-1
     */
    public double[][] data;
     ...
     * 误差停止阈值
     */
    public static final double STOP = 0.0000001;
    /**
     * 迭代次数阈值
     */
    public static final int NUMBER_ROUND = 1000000;
    ...
    public void init(int[] lens) {
        ...
        data = new double[layer][];
        for (int i = 0; i < layer; ++i) {
            data[i] = new double[lens[i]];
            for (int j = 0; j < lens[i]; ++j) data[i][j] = 0;
        }
    }
    ...
    public boolean backward(double[] labels) {
        ...
        for (int j = neurons.get(layer - 1).size() - 1; j >= 0; --j) {
            ...
            // 求导公式
            data[layer - 1][j] = -1 * (labels[j] - o) * o * (1 - o);
            // 更新倒数第二层和最后一层之间的权重
            for (int i = neurons.get(layer - 2).size() - 1; i >= 0; --i) {
                weight[layer - 2][i][j] -= RATE * data[layer - 1][j] * neurons.get(layer - 2).get(i).o;
            }
        }
        ...
        for (int l = layer - 3; l >= 0; --l) {
            ...
            for (int j = neurons.get(l + 1).size() - 1; j >= 0; --j) {
                ...
                for (int k = neurons.get(l + 2).size() - 1; k >= 0; --k) {
                    // C层残差通过权重weight传递过来
                    data[l + 1][j] += data[l + 2][k] * weight[l + 1][j][k];
                }
                ...
                for (int i = neurons.get(l).size() - 1; i >= 0; --i) {
                    // A层i神经元和B层j神经元权重
                    weight[l][i][j] -= RATE * data[l + 1][j] * neurons.get(l).get(i).o;
                }
            }
        }
        return true;
